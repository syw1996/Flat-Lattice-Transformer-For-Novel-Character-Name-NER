...
parser.add_argument('--status',choices=['train','test','predict'],default='train')
...
parser.add_argument('--batch', default=32, type=int) # 我修改了默认batch的大小
...
parser.add_argument('--dataset', default='ontonotes', help='weibo|resume|ontonotes|msra|novel')
...
if args.dataset == 'ontonotes':
    ...
    datasets,vocabs,embeddings = load_novel_ner(novel_ner_path,
                                                yangjie_rich_pretrain_unigram_path,
                                                yangjie_rich_pretrain_bigram_path,
                                                _refresh=refresh_data,
                                                _cache_fp=raw_dataset_cache_name,
...
# adam可以尝试用linear_schedule_with_warmup
# lrschedule_callback = LRScheduler(lr_scheduler=get_linear_schedule_with_warmup(optimizer, num_warmup_steps=1000, num_training_steps=30000))
...
if args.status == 'test': # 如果是做测试 
    ...
    model = torch.load("./model/best_Lattice_Transformer_SeqLabel_f_2022-03-01-20-07-10") # 加载训练好的模型
    tester = Tester(datasets['test'],
                    model,
                    metrics=metrics,
                    device=device)
    res = tester.test()
...
if args.status == 'predict': # 如果是做测试 
    # 因为这里使用的是fastNLP ，故加载模型的方法有些区别。直接load就可以了   
    model = torch.load("./model/best_Lattice_Transformer_SeqLabel_f_2022-03-01-20-07-10") # 加载训练好的模型
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
    predictor = Predictor(model)
    res = predictor.predict(datasets['test'])
    res_lst = [a.tolist()[0] for a in res['pred']]
    
    import json
    with open(os.path.join(novel_ner_path, "白袍总管/ner_result.json"), "w") as fw:
        json.dump(res_lst, fw)