...
@cache_results(_cache_fp='cache/novelNER_uni+bi', _refresh=False)
def load_novel_ner(path,unigram_embedding_path=None,bigram_embedding_path=None,index_token=True,
                   char_min_freq=1,bigram_min_freq=1,only_train_min_freq=0,char_word_dropout=0.01):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
    ...
    train_path = os.path.join(path,'novelNER.train_deseg')
    dev_path = os.path.join(path, 'novelNER.dev_deseg')
    test_path = os.path.join(path, '白袍总管/白袍总管NER_2nd_deseg')
